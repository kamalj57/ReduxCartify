...
import { decrementedquantity, incrementedquantity, itemsDeleted } from '../reducers/cartSlice';
...
const CartItem = ({ products }) => {
  ...
  return (
    <>
        ...
        <div className='flex flex-col items-center'>
          ...
          <div className="mt-5 w-full">
            ...
            <div className="font-bold text-lg text-black mt-3 flex items-center gap-3">
              <Minus size={20} className='text-gray-700 cursor-pointer border m-1 border-gray-300 rounded-sm' onClick={handleDecrementQuantity} />
              <div className='border border-gray-300 w-10 text-center'>{products.quantity}</div>
              <Plus size={20} className='text-gray-500 cursor-pointer border m-1 border-gray-300 rounded-sm' onClick={handleIncrementQuantity} />
            </div>
            <p className="font-bold text-lg text-black mt-3">
              Total Cost : ₹ {products.totalcost}
            </p>
            <div className='flex gap-10 mt-5'>
              <button className='flex gap-1 bg-red-500 w-44 h-12 items-center justify-center font-bold text-lg text-slate-100 rounded-sm' onClick={handleOpen}><Trash2 className='text-slate-50 cursor-pointer' />DELETE ITEM</button>
              <button className='flex gap-1 bg-orange-500 w-44 h-12 items-center justify-center font-bold text-lg text-slate-100 rounded-sm' onClick={handleOpenOrder}>PLACE ORDER</button>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-lg shadow-2xl w-1/3">
            ...
            <p className="mt-2 text-md">
              Are you sure you want to delete <b>{products.title}</b> from your cart?
            </p>
      ...
      <div className="relative bg-white rounded-3xl p-8 text-center w-1/3 shadow-2xl sm:p-12">
